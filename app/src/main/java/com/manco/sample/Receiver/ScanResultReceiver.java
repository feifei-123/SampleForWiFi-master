...
public class ScanResultReceiver extends BroadcastReceiver{
    ...
    public void onReceive(Context context, Intent intent) {
        Log.d("WIFIX","ScanResultReceiver - onReceive - "+intent.getAction());
        if (intent.getAction().equals(WifiManager.SCAN_RESULTS_AVAILABLE_ACTION)) {
            boolean isScanned = intent.getBooleanExtra(WifiManager.EXTRA_RESULTS_UPDATED, true);
            if (isScanned) {

                Log.d("WIFIX","ScanResultReceiver - onReceive - isScanned:"+isScanned+",getScapAP");
                List<AccessPoint> aps = WiFiHandler.instance().getScanAp();
                if (aps != null) {
                    Courier.send(Global.AP_AVAILABLE,WiFiHandler.mergeRelativeAPs(aps));//将aps 根据 ssid去重
                }
            }else {
                Log.d("WIFIX","ScanResultReceiver - onReceive - isScanned:"+isScanned+",do nothing");
            }
        }
    }
}
